...
import SwitchFrontLabel from '@/components/SwitchFrontLabel';
import TestSessionTable from '@/components/testSessionTable';
import TestSessionTableDetails from '@/components/testSessionTableDetails';
...
const draftTree = (trees, handlerChangeItemTree, index) => {
  const labelPass =
...
export default function ModalSelectTestCases({
  ...
  useMemo(() => {
    ...
    if (errorTestList) {
      if (errorTestList?.response?.data?.rtnCode === '9897') {
        router.reload();
      }
      setUserTestSets([]);
    }
  ...
  }, [statusGeneralProcess, isExecuted, modalRunning]);
        ...
        <div className="w-full">
          ...
          <div className="flex flex-col md:flex-row justify-between gap-4 mt-6 mb-3">
            <p className="font-medium">
              {paginate?.length} test cases selected
            </p>
