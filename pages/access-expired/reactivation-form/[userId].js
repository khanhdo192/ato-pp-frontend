    ...
    return (
      <div>
        ...
        <FormItemInputHorizontalLabel
          ...
          onChange={e => updateFieldValue(e.target.id, toUpper(e.target.value))}
          longCol
          isDisabled
        />
      ...
      <div>
        ...
        <ContainerCol_2 xtra="lg:gap-x-12">
          ...
          <FormItemSelectHorizontalLabel
            ...
            isDisabled={true}
          >
            <option value={getFieldValue('component')}>
              {getFieldValue('component') == 1 ? '3DS Server' : 'ACS'}
            </option>
          </FormItemSelectHorizontalLabel>
          ...
          <FormItemSelectHorizontalLabel
            ...
            isDisabled={true}
          >
            <option value={getFieldValue('protocolVersion')}>
              {getFieldValue('protocolVersion')}
            </option>
          </FormItemSelectHorizontalLabel>
      ...
      <div>
        ...
        <ContainerCol_2>
          ...
          <FormItemInputHorizontalLabel
            ...
            errorMsg={'Complete this field'}
            // TODO: define and use a gridTemplateColumns with: 6% 73%
          />
        </ContainerCol_2>
        <FormItemInputHorizontalLabel
    ...
    return (
      <div>
        <TextH1
          ...
          xtra=""
        />
        <div className="h-modal-test overflow-y-auto mb-6 border bg-gray-50 p-4 rounded-md">
          <TCDocument />
        </div>
        <FormItemCheckbox
          id="agree"
...
const initialFormFields = {
  ...
  applicationType: {
    ...
    step: 2,
    value: 5, // set "New operator compliance test request" as default
  },
  component: {
    validations: { isRequired: true },
    step: 2,
...
export default function RegisterPage({ props }) {
      ...
      try {
        ...
        if (response.rtnCode == '1') {
          ...
          if (response?.result?.isUpdateRequestBeenCompleted) {
            return router.push('/access-expired');
          }
          setLastStepNumber(4);
          setUpdatedRegisterForm(true);
          setOldEmail(dataUser.email || '');
          ...
          updateFieldValue('applicationType', 5);
  ...
  const renewAccess = async () => {
    try {
      ...
      const response = await postFetcher({ ...body, userId: queryParamUserId })(
      ...
      if (response?.rtnCode == '1') {
        ...
        //   const verifyResponse = await postFetcher({
        //     ...values,
        //     userId: queryParamUserId,
        //   })('/tester/auth/send-verify-code');
        //   if (verifyResponse?.rtnCode == '1') {
        //     setIsVerifyEmailModalOpen(true);
        //     setUserID(verifyResponse?.result?.userId);
        //   } else {
        //     setErrorMsg(
        //       verifyResponse?.result?.message || 'Something went wrong'
        //     );
        //   }
        //   setSubmitted(false);
        // } else {
