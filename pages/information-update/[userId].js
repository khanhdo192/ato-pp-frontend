...
export default function InformationUpdatePage({ user }) {
  ...
  const { user: userInfo, mutateUser } = useUser({ redirectTo: '/login' });
  ...
  const getFormValues = () => {
    ...
    return (formValues = { ...formValues, sdkInformation });
  };
    ...
    const loadData = async () => {
      try {
        ...
        if (response.rtnCode == '1') {
          ...
          updateFieldValue('applicationType', 4);
  ...
  const informationUpdate = async () => {
    try {
      ...
      body.userId = queryParamUserId;
      body.hasUpdateRequestBeenCompleted = true;

      const res = await postFetcher(body)('/tester/auth/update-user-form');
      if (res?.rtnCode == '1') {
        setSubmitted(false);
            ...
            <div className="flex justify-between">
              ...
              <BtnV2
                ...
                isDisable={
                  ...
                  (currentStep === lastStepNumber && !agree)
                }
                onClick={() => onNext()}
              />
            </div>
