...
export default function RegisterPage({ user }) {
  ...
  const { mutateUser, user: userInfo } = useUser({ redirectTo: '/login' });
    ...
    const loadData = async () => {
      try {
        ...
        if (response.rtnCode == '1') {
          ...
          updateFieldValue('applicationType', 3);
          ...
          updateFieldValue('protocolVersion', '2.2.0');
          updateFieldValue(
            'approvalExpirationDate',
  ...
  const newProtocolVersion = async () => {
    try {
      if (!!submitted) return;
      setSubmitted(true);
